...
const Profile = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      const res = await api.put("/auth/profile", { name: form.name, email: form.email, bio: form.bio });
  ...
  return (
    <PageWrapper title="My Profile">
      <div className="max-w-2xl mx-auto space-y-4">
        {/* Profile Card */}
        <div className="rounded-2xl p-6" style={{ backgroundColor: cardBg, border: `1px solid ${border}`, boxShadow: isDark ? "0 4px 24px rgba(0,0,0,0.3)" : "0 4px 24px rgba(0,0,0,0.06)" }}>
          <div className="flex items-start justify-between mb-5">
            <div className="flex items-center gap-4">
              <div className="relative">
                <img src={user?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.name}`} alt={user?.name} className="w-16 h-16 rounded-2xl" style={{ border: `3px solid ${roleColor}` }} />
                <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center" style={{ backgroundColor: "#22c55e", border: `2px solid ${cardBg}` }}>
            ...
            <button onClick={() => { setEditing(!editing); setChangingPassword(false); }}
            ...
            {[
              ...
              <div key={key}>
                ...
                <div className="relative">
                  ...
                  <input type={type} value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })} disabled={!editing}
